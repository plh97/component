...
const eventProxy = args => {
  ...
  if (event === 'change') {
    ...
    const handleAllEvent = e => {
      ...
      let filterList = addArrProp$1(allList).filter(list => {
...
const eventProxy$1 = args => {
  ...
  if (event === 'click') {
    ...
    const handleAllEvent = e => {
      let searchValue = e.target.value.replace(/([.?*+^$[\]\\(){}|-])/g, "\\$1");
      const allList = document.querySelector('#sec-table-tb-container').children;
      let filterList = addArrProp$2(allList).filter(list => {
      ...
      openList.forEach(dom => {
        ...
        if (isShowAllInPath) {
          ...
          domToggleAnimation$2({
            ...
            animationDuration: '0.3s',
            animationFillMode: 'forwards',
            animationName: [styles$8.rotate90, styles$8['rotate-90']]
          });
          const listContainer = isShowAllInPath.parentElement;
          domToggleAnimation$2({
            dom: listContainer,
            animationDuration: '0.3s',
            animationFillMode: 'forwards',
            animationName: [styles$8.slideout1, styles$8.slidein1]
          });
        }
      });

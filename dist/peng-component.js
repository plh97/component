...
const putDataToSecTable = ({ data, tableHead }) => __async(function* () {
  ...
  data.forEach((row, i) => {
    ...
    addArrProp$2(tableHead).forEach(dom => {
...
const treeTable = args => __async(function* () {
  ...
  document.body.appendChild(mask);
  // await sleep(300);
  const getTableHTML = yield fetchData$1({
  ...
  let tableHead = createElementFromHTML$1(getTableHTML.data).querySelectorAll('thead tr th');
  addArrProp$2(tableHead).forEach(dom => {
    console.log(dom);
...
const eventProxy$2 = args => {
  ...
  if (event === 'click') {
    const handleAllEvent = e => {
      ...
      // 是否弹出表格
      const isTableContainer = isIdInPathFunc$2({
        path,
        id: 'thr-table'
      });
      const isShow = isIdInPathFunc$2({
        path,
        id: 'show'
      });
      if (isTableContainer) {
...
const thrTableObserver$1 = ({ treeStyles }) => {
  ...
  const observer = new MutationObserver(() => {
    ...
    addArrProp$3(document.querySelectorAll(`.${treeStyles.tree} .${treeStyles.active}`)).forEach(dom => {
...
const tree$1 = args => __async(function* () {
  ...
  const tree$$1 = tree({ data: data.title, beforeSelect, selectModel: "radio" });
...
const selectBeforeFunc$2 = args => {
  ...
  const contents = document.querySelectorAll('#sec-table-tb-container >div');
  addArrProp$4(contents).forEach(content => {
    beforeSelect.forEach(select => {
      const name = content.querySelector(`.${styles$11.name}`);
      if (name.innerText === select) {
        content.click();
      }
    });
...
const putDataToSecTable$1 = ({ data, tableHead }) => __async(function* () {
  // 将数据传入data之前先清空 container
  let secTableInputs = document.querySelector('#sec-table-tb-container');
  secTableInputs = Array.prototype.slice.call(secTableInputs);
  secTableInputs.map(input => input.parentElement.remove());
  data.forEach((row, i) => {
    ...
    addArrProp$4(tableHead).forEach(dom => {
      const id = dom.dataset.field;
...
const eventProxy$3 = args => {
  const { event } = args;
  const domAddEvent = args.domAddEvent || document.querySelector(`.${styles$11['component-mask']}`);
  if (event === 'click') {
    const handleAllEvent = e => {
      // empty
      ...
      if (isEmptyDom) {
        let inputs = isEmptyDom.parentElement.parentElement.querySelectorAll(`.${styles$11['tb-container']} .${styles$11.select}`);
        inputs.forEach(input => {
          if (input.parentElement.style.display !== 'none') {
    ...
    const handleAllEvent = e => {
      ...
      if (isSelectReverseDom) {
        const inputs = isSelectReverseDom.parentElement.parentElement.parentElement.querySelectorAll(`.${styles$11['tb-container']} .${styles$11.select}`);
        inputs.forEach(input => {
          if (input.parentElement.style.display !== 'none') {
    ...
    const handleAllEvent = e => {
      const searchValue = e.target.value.replace(/([.?*+^$[\]\\(){}|-])/g, '\\$1');
      const allList = document.querySelector('#sec-table-tb-container').children;
      const filterList = addArrProp$4(allList).filter(list => {
      ...
      addArrProp$4(allList).forEach(dom => dom.style.display = 'none');
      addArrProp$4(filterList).forEach(dom => dom.style.display = 'flex');
    };
    domAddEvent.addEventListener(event, handleAllEvent, false);
  }
};
  ...
  const observer = new MutationObserver(() => {
    ...
    inputGroup.map((input, i) => {
      ...
      addEvent$1({
        dom: newChild,
        envet: 'click',
        func: e => e.path.filter(_e => _e.className === styles$11.tb)[0].remove()
      });
...
const Table = args => __async(function* () {
  ...
  document.body.appendChild(mask);
  // await sleep(300);
  const getTableHTML = yield fetchData$2({
    url: "https://www.kingubo.cn/frontend/api/pc/getSelectTemplate/" + pars.tempid,
    data: ``,
    header: {
      method: "GET",
  ...
  let tableHead = createElementFromHTML$2(getTableHTML.data).querySelectorAll('thead tr th');
  addArrProp$4(tableHead).forEach(dom => {
    console.log(dom);
    const id = dom.dataset.field;
  ...
  let btns = mask.querySelectorAll(`.${styles$11['component-table']} button`);
  btns = Array.prototype.slice.call(btns);
  yield btnAddevent$2({
    btns, mask, data, next
  });
  // 添加观察者
  yield thrTableObserver$2();
  // all event proxy
  yield eventProxy$3({
    event: 'click'
  });
...
const eventProxy$5 = args => {
  ...
  if (event === 'click') {
    const handleAllEvent = e => {
      ...
      // 为第三个表格每一个列表添加点击事件, 就是点击第二个表格，由第二个表格触发第三个表格事件
      document.querySelectorAll(`#thr-table-tb-container .${styles$13.tb}`).forEach(dom => {
      ...
      const bodyContainer = document.querySelector(`.${styles$13['body-container']}`);
      // 是否弹出表格
...
const thrTableObserver$4 = ({ treeStyles }) => {
  const treeContainer = document.querySelector('#tree-container');
  const thrTableContainer = document.querySelector('#thr-table-tb-container');
  const MutationObserver = window.MutationObserver || window.WebKitMutationObserver || window.MozMutationObserver;
  const observer = new MutationObserver(() => {
    document.querySelector('#thr-table-tb-container').innerHTML = '';
    addArrProp$6(document.querySelectorAll(`.${treeStyles.tree} .${treeStyles.active}`)).forEach(dom => {
      const jsonData = JSON.parse(dom.dataset.json);
      const div = document.createElement('label');
...
const tree$3 = args => __async(function* () {
  const {
    data,
    next,
    beforeSelect
  ...
  const tree$$1 = tree({ data, beforeSelect, selectModel });
  const treeDom = tree$$1.container;
  const treeStyles = tree$$1.styles;
  mask.querySelector('#side').appendChild(treeDom);
  document.body.appendChild(mask);
  ...
  // 添加观察者
  yield thrTableObserver$4({ selectModel, treeStyles });
  // all event proxy
  yield eventProxy$5({
    event: 'click',
    selectModel,
